...
export default function AttendanceAnalysis() {
  ...
  useEffect(() => {
    ...
    setLoading(true);
    fetch(`http://localhost:5000/api/attendance-analysis/summary?classId=${selectedClass}`)
      .then(res => res.json())
  ...
  useEffect(() => {
    if (selectedStudent) {
      fetch(`http://localhost:5000/api/attendance-analysis/student/${selectedStudent}`)
        .then(res => res.json())
