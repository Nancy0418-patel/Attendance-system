    ...
    const fetchStudentData = async () => {
      ...
      try {
        ...
        if (studentProfileData.classId && studentProfileData.classId.className) {
          ...
          const timetableResponse = await fetch(`http://localhost:5000/api/timetable/${encodeURIComponent(studentProfileData.classId.className)}/${dayOfWeek}`);
          ...
          const attendanceResponse = await fetch(`http://localhost:5000/api/attendance/${studentProfileData.userId}`, {
            headers: {
              Authorization: `Bearer ${token}`,
            },
          });
